...
export const KNOWN_PRESETS_BY_RUNNER: Record<RunnerId, readonly string[]> = {
  ...
  // pi picks a model with the same `provider/model` convention as opencode, so its presets
  // deliberately overlap opencode's — a shared id is one runner's own preset for BOTH, which
  // `modelConflictsWithRunner` treats as no conflict (it only rejects ids exclusive to another
  // runner). Same shape as the composer's `MODELS_BY_RUNNER.pi`.
  pi: ['anthropic/claude-opus-4-8', 'anthropic/claude-sonnet-5', 'openai/gpt-5.1'],
};
