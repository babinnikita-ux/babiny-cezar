...
// `POST /api/runs/:id/open-in` (#429) — the chosen local app/target id. Matched
// against a downstream allowlist; a short bound is plenty for an editor id.
...
export function createApp(deps: ServerDeps): Hono {
  ...
  app.post('/api/runs/:id/open-in', async (c) => {
    ...
    // Follows the safeParse convention (#429); the downstream allowlist match
    // is the real injection guard, this just validates the shape.
    const parsed = openInSchema.safeParse(await c.req.json().catch(() => null));
    if (!parsed.success) return c.json({ error: 'target required' }, 400);
