...
import { buildInstallLines, computeSnapshot, stampManifests, type SnapshotContext } from './snapshot.js';
...
describe('stampManifests', () => {
  const root = { name: '@open-mercato/cezar', version: '0.1.5', bin: { cezar: 'dist/index.js' } };
  const alias = {
    name: 'cezar-cli',
    version: '0.1.5',
  ...
  it('stamps both manifests to the snapshot version and pins the alias exact', () => {
    const stamped = stampManifests(root, alias, '0.1.5-pr482.123');
    expect(stamped.root.version).toBe('0.1.5-pr482.123');
  ...
  it('does not mutate the inputs and passes unrelated fields through', () => {
    const stamped = stampManifests(root, alias, '0.1.5-main.9');
