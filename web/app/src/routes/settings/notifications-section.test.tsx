...
describe('the notifications section', () => {
  ...
  it('a denial GIVEN AT the enable prompt surfaces the same warning', async () => {
    ...
    renderSection();
    fireEvent.click(toggle());
    await waitFor(() => expect(document.querySelector('[data-slot="notifications-denied"]')).not.toBeNull());
  });
  ...
  it('no Notification API at all: the switch is disabled and the section says so', async () => {
    ...
    expect(document.querySelector('[data-slot="notifications-unsupported"]')?.textContent).toContain(
      'does not support',
    );
  });
});
