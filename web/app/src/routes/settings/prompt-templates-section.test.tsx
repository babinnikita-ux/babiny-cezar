...
const resetButton = () =>
  document.querySelector<HTMLButtonElement>('[data-action="prompt-templates-reset"]')!;
...
describe('the prompt templates section', () => {
  it('an untouched ui-state renders the built-in templates, Save disabled (nothing edited yet)', async () => {
    ...
    expect(rows().map((row) => row.dataset.template)).toEqual(DEFAULT_PROMPT_TEMPLATES.map((t) => t.id));
...
describe('assigning a template to a skill', () => {
  ...
  it('assigning shows a chip and Save PUTs the skills alongside the template', async () => {
    ...
    await waitFor(() =>
      expect(
        rows()[0]!.querySelector('[data-slot="prompt-template-skill-chip"][data-skill="om-fix"]'),
      ).not.toBeNull(),
    );
