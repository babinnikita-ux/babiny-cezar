  ...
  vi.stubGlobal(
    ...
    vi.fn(async (input: RequestInfo | URL, init: RequestInit = {}) => {
      ...
      if (method === 'POST' && path.endsWith('/start')) {
        ...
        inbox = inbox.map((item) => (item.id === id ? { ...item, startedTaskId: STARTED_RUN.id } : item));
...
describe('the inbox card list', () => {
  ...
  it('says "source task deleted" when the source run is gone (legacy honesty rule)', async () => {
    ...
    expect(card.querySelector('[data-slot="todo-source-gone"]')?.textContent).toBe('source task deleted');
  });
...
describe('Run — backend selection (#401)', () => {
  ...
  it('an untouched card honors Settings → Agents defaultModels — the one real behavior change', async () => {
    ...
    const card = cards()[0]!;
    await waitFor(() =>
      expect(card.querySelector('[data-slot="model-pill"]')?.textContent).toContain('opus'),
    );
  ...
  it('a model pick rides along with the runner', async () => {
    ...
    await waitFor(() => expect(startBody(sent, 't1')).toEqual({ runner: 'codex', model: 'gpt-future' }));
  });
...
describe('Acknowledge', () => {
  it('replaces Run for a note and DELETEs it without starting a task', async () => {
    ...
    expect(note.querySelector('[data-action="todo-acknowledge"]')?.textContent).toContain('Acknowledge');
...
describe('empty and error states', () => {
  ...
  it('an all-started inbox is an empty inbox — the audit trail is not a card list', async () => {
    ...
    await waitFor(() => expect(document.querySelector('[data-slot="centered-state"]')).not.toBeNull());
