...
describe('reduceThread — mixed v1+v2 files (the dedup rule)', () => {
  ...
  const mixed: RunEvent[] = [
    ...
    line(6, 'item.started', {
      item: { kind: 'message', id: 'item_1', role: 'assistant', text: md },
      stepId: 'task',
    }),
    line(7, 'item.completed', {
      item: { kind: 'message', id: 'item_1', role: 'assistant', text: md },
    ...
    line(17, 'item.started', {
      item: { kind: 'message', id: 'item_2', role: 'assistant', text: 'Follow-up #1 received.' },
      stepId: 'task',
    }),
    line(18, 'item.completed', {
      item: { kind: 'message', id: 'item_2', role: 'assistant', text: 'Follow-up #1 received.' },
      stepId: 'task',
    }),
...
describe('reduceThread — live-stream mechanics', () => {
  ...
  it('does not mutate the input events (deltas land on clones)', () => {
    const started = line(2, 'item.started', {
      item: { kind: 'message', id: 'm1', role: 'assistant', text: '' },
    });
    reduceThread([
      line(1, 'turn.started', { turnId: 't' }),
      started,
      line(3, 'item.delta', { itemId: 'm1', field: 'text', delta: 'x' }),
    ]);
  ...
  it('strips a trailing CEZ:DONE from assistant messages — v2 carries it raw', () => {
    const events: RunEvent[] = [
      ...
      line(2, 'item.completed', {
        item: { kind: 'message', id: 'm1', role: 'assistant', text: 'All done.\n\nCEZ:DONE' },
      }),
    ];
    const { turns } = reduceThread(events);
    ...
    const events: RunEvent[] = [
      ...
      line(2, 'item.completed', {
        item: {
          ...
          id: 'm1',
          role: 'assistant',
          text: 'Spawned the reviewer, waiting on it.\n\nCEZ:MONITORING',
        },
      }),
    ];
    const { turns } = reduceThread(events);
  ...
  it('strips CEZ:PR/CEZ:ISSUE/CEZ:TITLE marker lines but keeps prose mentions (spec 2026-07-18-task-ref-markers)', () => {
    ...
    const { turns } = reduceThread(events);
    expect((turns[0]!.items[0] as UiMessageItem).text).toBe(
      'Opened the PR.\nI will keep CEZ:PR=442 updated.',
    );
  });
...
describe('latestPlanEntries — the dock takes the newest snapshot across turns', () => {
  ...
  it('v1-only fallback: a TodoWrite tool-call input yields the plan for old transcripts', () => {
    const todos = [
      {
        content: 'Patch middleware redirect',
        status: 'completed',
        activeForm: 'Patching middleware redirect',
      },
...
describe('threadFooter', () => {
  const cases = [
    ...
    [
      'review',
      undefined,
      { state: 'closed', tone: 'dim', label: 'Session closed — waiting for your review' },
    ],
...
describe('reduceThread — AskUser cards (#473)', () => {
  ...
  it('drops an ask.requested with no valid questions', () => {
    expect(
      allItems([line(1, 'ask.requested', { requestId: 'x', questions: [] })]).some((i) => i.kind === 'ask'),
    ).toBe(false);
  });
