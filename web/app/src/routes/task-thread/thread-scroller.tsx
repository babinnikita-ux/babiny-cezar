    ...
    <div ref={controls.attachContent} data-slot="thread-rows" data-virtualized="false">
      {rows.map((row) => (
        ...
        <div
          ...
          className="flex flex-col pb-3.5 [contain-intrinsic-block-size:auto_3rem] [content-visibility:auto]"
        >
          {row.node}
        </div>
      ))}
...
function VirtualRows({
  ...
  return (
    ...
    >
      ...
      >
        {rows.map((row) => (
          <div key={row.key} data-slot="thread-row" className="flex flex-col pb-3.5">
            {row.node}
          </div>
        ))}
      </Virtualizer>
