...
describe('collectSubagents — a terminal run', () => {
  ...
  it('never carries a stalled agent forward into a later fan-out', () => {
    const turns = [turn('turn-1', [task('a', 'running')]), turn('turn-2', [task('b', 'running')])];
...
describe('collectSubagents — activity line', () => {
  ...
  it('truncates a long line to a single ellipsised readout', () => {
    ...
    const agents = collectSubagents([turn('turn-1', [task('a', 'running'), childText('c1', 'a', long)])]);
...
describe('collectSubagents — agentType', () => {
  it.each([
    ...
    const agents = collectSubagents([turn('turn-1', [task('a', 'running', { input: { [key]: value } })])]);
  ...
  it('is undefined when the input carries no type (codex) or a non-string one', () => {
    ...
    expect(
      collectSubagents([turn('turn-1', [task('a', 'running', { input: { subagent_type: 42 } })])])[0]!
        .agentType,
    ).toBeUndefined();
    expect(
