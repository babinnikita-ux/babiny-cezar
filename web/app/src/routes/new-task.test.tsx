...
function serve(
  ...
  vi.stubGlobal(
    ...
    vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url === '/api/models?runner=codex')
        return json({
          runner: 'codex',
      ...
      if (url === '/api/config' && method === 'PUT')
        return json({
          baseBranch: (body as { baseBranch: string | null }).baseBranch,
          defaultRunner: 'claude',
        });
...
describe('the hero surface', () => {
  it('renders the mockup hero: title, subtitle, twinkles, and focus lands in the textarea', async () => {
    ...
    expect(
      screen.getByText('Runs in an isolated worktree — review everything before it lands.'),
    ).toBeTruthy();
...
describe('submit', () => {
  ...
  it('picking a model and ×2 variants rides along; {runs} answer navigates to the FIRST variant', async () => {
    ...
    await waitFor(() =>
      expect((document.querySelector('[data-slot="variants-pill"]') as HTMLElement).textContent).toContain(
        '×2',
      ),
    );
  ...
  it('defaults follow-up generation on, but posts and remembers an explicit opt-out', async () => {
    ...
    renderNewTask();
    await pillReady();
    const toggle = document.querySelector('[data-slot="generate-followups-toggle"]') as HTMLButtonElement;
  ...
  it('uses the remembered follow-up preference when the draft has no choice', async () => {
    ...
    await pillReady();
    expect(
      document.querySelector('[data-slot="generate-followups-toggle"]')?.getAttribute('aria-checked'),
    ).toBe('false');
  });
  ...
  const followupsToggle = () => document.querySelector('[data-slot="generate-followups-toggle"]');
...
describe('drafts and prefill', () => {
  ...
  it('carries todoId even when follow-up generation is switched off', async () => {
    ...
    renderNewTask('/new?skill=deploy&ref=ship%20it&todo=t1');
    await pillReady('deploy');
    fireEvent.click(document.querySelector('[data-slot="generate-followups-toggle"]') as HTMLElement);
    await startTask();
...
const stepIds = () =>
  [...document.querySelectorAll('[data-slot="plan-step"]')].map((el) => el.getAttribute('data-step-id'));
...
describe('the Start | Plan first toggle', () => {
  ...
  it('disables the Autonomous toggle in plan mode (planning is interactive)', async () => {
    ...
    renderNewTask();
    await pillReady();
    const autonomous = () => document.querySelector('[data-slot="autonomous-toggle"]') as HTMLButtonElement;
...
describe('the plan flow', () => {
  it('submit in plan mode POSTs /api/plan (never /api/runs) and opens the review overlay', async () => {
    ...
    expect(document.querySelector('[data-slot="plan-task"]')?.textContent).toBe('Tighten the flaky suite');
  ...
  it('✕ removes a step and ↑/↓ reorder (the touch-honest path)', async () => {
    ...
    expect((screen.getByRole('button', { name: 'Move step 2 down' }) as HTMLButtonElement).disabled).toBe(
      true,
    );
  });
