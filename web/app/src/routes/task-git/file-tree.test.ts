...
describe('buildFileTree', () => {
  ...
  it('compacts single-child folder chains into one joined row — but never the root', () => {
    const root = buildFileTree([file('web/app/src/main.tsx'), file('web/app/src/lib/x.ts')]);
    // web → app → src is a pure chain (no files on the way): one "web/app/src" row.
    expect(root.dirs).toHaveLength(1);
    const chain = root.dirs[0]!;
    expect(chain.name).toBe('web/app/src');
    expect(chain.path).toBe('web/app/src');
  ...
  it('carries status/binary through to the leaves', () => {
    const root = buildFileTree([file('img/logo.png', { status: 'added', binary: true, adds: 0, dels: 0 })]);
