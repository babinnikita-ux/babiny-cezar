...
import type { WorktreeEntry } from '@/api/types';
  ...
  return (
    <Pane className={className}>
      ...
      >
        ...
        <span className="ml-auto shrink-0 tabular-nums text-soft-foreground">
        ...
        <div key={index} className="flex px-4">
          ...
          <span className="min-w-0 flex-1 whitespace-pre pr-4">
            {line.map((token, tokenIndex) => (
              <span key={tokenIndex} style={token.color !== undefined ? { color: token.color } : undefined}>
                {token.content}
              </span>
            ))}
          </span>
