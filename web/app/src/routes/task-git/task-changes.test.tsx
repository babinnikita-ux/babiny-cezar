...
describe('the Changes tab route', () => {
  ...
  it('hosted mode (localHandoff: false) hides the overflow menu entirely', async () => {
    stubFetch({
      'GET /api/health': () => jsonResponse({ ...HEALTH, capabilities: { localHandoff: false } }),
    });
    renderChangesRoute();
  ...
  it('local mode offers the terminal handoff in the overflow menu', async () => {
    stubFetch();
    renderChangesRoute();
    await waitFor(() => expect(document.querySelector('[aria-label="More git actions"]')).not.toBeNull());
  });
});
  ...
  it('disabled entries render disabled with the policy reason as the tooltip', () => {
    renderToolbar({
      primary: {
        ...
        label: 'Commit',
        enabled: false,
        reason: 'Commit unavailable — no changes to commit',
      },
      secondary: [
