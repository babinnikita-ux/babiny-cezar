...
describe('parseNewTaskParams', () => {
  const cases: Array<[name: string, search: string, expected: NewTaskParams]> = [
    ...
    [
      'skill only (the "open the composer with this skill" bookmarklet)',
      '?skill=om-fix',
      { ...empty, skill: 'om-fix' },
    ],
    ...
    [
      'whitespace around skill/ref is trimmed',
      '?skill=%20om-fix%20&ref=%20x%20',
      { ...empty, skill: 'om-fix', ref: 'x' },
    ],
    ...
    [
      'the inbox prefill link',
      '?skill=om-fix&ref=fix+it&todo=t1',
      { ...empty, skill: 'om-fix', ref: 'fix it', todo: 't1' },
    ],
    ...
    [
      'no todo is the empty string, not undefined',
      '?skill=om-fix&auto=1&key=k',
      { ...empty, skill: 'om-fix', auto: true, key: 'k' },
    ],
  ];
