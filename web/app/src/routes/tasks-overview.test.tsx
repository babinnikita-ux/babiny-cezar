...
const cellsOf = (id: string): string[] =>
  [...(tableRow(id)?.querySelectorAll('td') ?? [])].map((td) => td.textContent ?? '');
...
describe('TasksOverview — the table', () => {
  ...
  it('gives the title a real link, so keyboards and middle-clicks work too', () => {
    ...
    expect(
      within(tableRow('k1') as HTMLElement)
        .getByRole('link', { name: 'Keyboard reachable' })
        .getAttribute('href'),
    ).toBe('/tasks/k1');
  });
...
describe('TasksOverview — usage cells', () => {
  ...
  it('emphasizes live CPU/Mem for a running run, from the usage stream', () => {
    ...
    act(() =>
      FakeEventSource.last?.emit('usage', JSON.stringify({ project: 'boot', usage: { live1: SAMPLE } })),
    );
  ...
  it('shows a finished run its dimmed peaks, and never a live sample', () => {
    ...
    act(() =>
      FakeEventSource.last?.emit('usage', JSON.stringify({ project: 'boot', usage: { done1: SAMPLE } })),
    );
...
describe('TasksOverview — mobile cards and FAB', () => {
  ...
  it('packs a card with the pill, the meta row and the age', () => {
    ...
    expect(c.querySelector('[data-slot="pr-chip"]')?.getAttribute('href')).toBe(
