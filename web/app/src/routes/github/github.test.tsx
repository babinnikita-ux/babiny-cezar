...
function stubFetch(overrides: Record<string, () => Response> = {}): SentRequest[] {
  ...
  vi.stubGlobal(
    ...
    vi.fn(async (input: RequestInfo | URL, init: RequestInit = {}) => {
      ...
      if (method === 'GET' && path === '/api/models?runner=codex')
        return jsonResponse({
          runner: 'codex',
...
const promptField = () => document.querySelector<HTMLTextAreaElement>('[data-slot="gh-custom-prompt"]')!;
...
describe('the GitHub tab lists', () => {
  ...
  it('a row deep link selects that item and renders its detail', async () => {
    ...
    expect(
      document.querySelector('[data-slot="gh-row"][data-number="139"]')?.getAttribute('aria-current'),
    ).toBe('page');
...
describe('remembering the last-selected tab (#417)', () => {
  it('clicking Pull requests persists the choice via PUT /api/ui-state', async () => {
    ...
    await waitFor(() =>
      expect(sent.some((request) => request.method === 'PUT' && request.path === '/api/ui-state')).toBe(true),
    );
...
describe('the comment thread', () => {
  ...
  const EVT = {
    committed: {
      ...
      sha: SHA,
      message: 'bound the timeline page loop',
    },
    labeled: {
      id: 'evt-1',
    ...
    crossRef: {
      id: 'evt-3',
      kind: 'cross-referenced' as const,
      actor: 'octocat',
      createdAt: '2026-01-05T00:00:00Z',
      refNumber: 520,
      refTitle: 'Sibling work',
      refIsPr: true,
      url: 'https://github.com/o/r/pull/520',
    },
  };
...
describe('the unavailable forge state', () => {
  it('renders the server reason and the gh hint, and Try again refetches with refresh=1', async () => {
    ...
    await waitFor(() => expect(sent.some((request) => request.path === '/api/github?refresh=1')).toBe(true));
  });
});
...
describe('the hand-to-agent pickers (#385)', () => {
  it('the workflow dropdown lists, filters, selects — and re-selecting deselects', async () => {
    ...
    fireEvent.click(document.querySelector('[data-workflow="ship-it"]')!);
    await waitFor(() =>
      expect(document.querySelector('[data-slot="gh-workflow-trigger"]')?.textContent).not.toContain(
        'ship-it',
      ),
    );
  });
  ...
  it('the skills dropdown is project-first with bold project rows (#377), and keeps selection visible as chips', async () => {
    ...
    fireEvent.click(document.querySelector('[data-slot="gh-skills-trigger"]')!);
    await waitFor(() => expect(document.querySelectorAll('[data-slot="gh-skill-option"]')).toHaveLength(3));
    ...
    await waitFor(() => expect(document.querySelectorAll('[data-slot="gh-skill-option"]')).toHaveLength(1));
  ...
  it('the eye opens the read-only skill preview (shared detail component) without toggling the row', async () => {
    ...
    fireEvent.click(
      document.querySelector(
        '[data-slot="gh-skill-option"][data-skill="om-fix"] [data-slot="gh-skill-view"]',
      )!,
    );
    ...
    expect(preview.querySelector('[data-slot="skill-preview-manage"]')?.getAttribute('href')).toBe(
      '/skills?skill=om-fix',
    );
  });
...
describe('the skills dropdown frequency sort (#408 item 1, re-tiered by #519)', () => {
  ...
  it('a run started while ui-state is unavailable skips the bump rather than wiping the map', async () => {
    ...
    expect(sent.some((request) => request.method === 'PUT' && request.path === '/api/ui-state')).toBe(false);
  });
...
describe('⌘/Ctrl+Enter submits the follow-up composer (#408 item 5)', () => {
  ...
  it('a bare Enter does NOT submit — it is a multi-line instructions box', async () => {
    ...
    expect(sent.some((request) => request.method === 'POST' && request.path === '/api/runs')).toBe(false);
  });
...
describe('the follow-up prompt template menu (#413)', () => {
  ...
  async function chooseTemplate(id: string): Promise<void> {
    ...
    await waitFor(() => {
      if (textarea().value === before) throw new Error(`template "${id}" has not been inserted yet`);
    });
  }
  ...
  it('a user-edited ui-state templates list replaces the built-ins in the menu', async () => {
    stubFetch({
      'GET /api/ui-state': () =>
        jsonResponse({
          promptTemplates: [{ id: 'custom-1', label: 'My snippet', text: 'Custom instructions.' }],
        }),
    });
    await openDetail();
