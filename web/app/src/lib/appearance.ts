 ...
 * `web/app/index.html` can stamp `data-accent`/`data-density` before first paint — the same
 * no-flash trick the theme uses. When the server answers, its value wins and the mirror is
 * rewritten.
 *
 * IMPORTANT: `web/app/index.html`'s inline script duplicates `applyAppearance`'s stamping in
