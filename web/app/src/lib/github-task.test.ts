...
describe('githubTaskPrompt', () => {
  ...
  it('a PR reads "Address GitHub pull request"', () => {
    expect(githubTaskPrompt(item({ kind: 'pr', number: 7 }))).toContain('Address GitHub pull request #7:');
  });
...
describe('githubTaskRef', () => {
  ...
  it('a PR reads "Address GitHub pull request"', () => {
    expect(githubTaskRef(item({ kind: 'pr', number: 7 }))).toContain('Address GitHub pull request #7:');
  });
});
...
describe('applyItemTokens', () => {
  it('substitutes {{number}}, {{title}} and {{url}}, case- and space-insensitively', () => {
    expect(applyItemTokens('rebase {{number}} onto develop', item())).toBe('rebase #142 onto develop');
