...
describe('useSkills', () => {
  it('renders the fast catalog, then converges when the cold team cache is ready', async () => {
    ...
    await waitFor(() => expect(result.current.data?.map((skill) => skill.name)).toEqual(['local', 'om-fix']));
...
describe('useRunChanges', () => {
  it('opts out of the no-poll defaults so a finished run’s tab refetches on focus (#488)', async () => {
    ...
    const options = client.getQueryCache().find({ queryKey: queryKeys.runs.changes('run-1') })
      ?.observers[0]?.options;
