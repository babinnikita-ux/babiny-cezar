...
describe('AddProjectDialog', () => {
  ...
  it('marks an already-registered folder and navigates to it when the server answers 409', async () => {
    ...
    expect(
      within(rows().getByText('cezar').closest('button') as HTMLElement).getByText('already added'),
    ).toBeTruthy();
  ...
  it('shows a register refusal verbatim and stays put', async () => {
    serve({
      browse: { '': json(HOME) },
      register: () =>
        json({ error: 'not a project folder: ~ is your home directory or a cezar task worktree' }, 400),
    });
