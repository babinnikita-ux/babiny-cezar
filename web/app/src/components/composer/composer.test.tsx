...
const SKILLS: Skill[] = [
  {
    name: 'global-deploy',
    description: 'Deploy from anywhere',
    body: '',
    path: '/g/global-deploy.md',
    source: 'global',
  },
...
function renderComposer(props: Partial<ComposerProps> = {}, uiState: Record<string, unknown> | null = {}) {
...
describe('failure restores the draft (nothing the user typed is ever lost)', () => {
  it('rejection → danger toast with the server words + the text back in the box', async () => {
    const onSubmit = vi.fn(() =>
      Promise.reject(new Error('session closed — the agent is no longer listening')),
    );
...
describe('@ file mentions — the provider seam (R5 upgrades the source, not the composer)', () => {
  ...
  it('a provider with nothing yet says so honestly instead of faking results', async () => {
    ...
    type(textarea, '@x');
    expect(await screen.findByText(/No files seen in this session yet/)).toBeTruthy();
  });
