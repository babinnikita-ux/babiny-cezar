...
describe('detectTrigger — when does typing open the menu (#380)', () => {
  const table: Array<{
    ...
    {
      name: 'a lone / at the start opens with an empty query',
      text: '/',
      caret: 1,
      expected: { trigger: '/', start: 0, query: '' },
    },
    {
      name: 'typing after the / becomes the query',
      text: '/om-f',
      caret: 5,
      expected: { trigger: '/', start: 0, query: 'om-f' },
    },
    {
      name: '/ after a space is a word boundary',
      text: 'use /rev',
      caret: 8,
      expected: { trigger: '/', start: 4, query: 'rev' },
    },
    {
      name: '/ after a newline is a word boundary',
      text: 'line\n/x',
      caret: 7,
      expected: { trigger: '/', start: 5, query: 'x' },
    },
    {
      name: '@ works the same',
      text: 'see @src',
      caret: 8,
      expected: { trigger: '@', start: 4, query: 'src' },
    },
    ...
    {
      name: 'caret INSIDE the token still counts, query up to the caret',
      text: '/abcd',
      caret: 3,
      expected: { trigger: '/', start: 0, query: 'ab' },
    },
