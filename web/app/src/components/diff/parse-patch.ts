...
function walkHunk(
  hunk: Hunk,
  onContext: (line: HunkLine) => void,
  onBlock: (block: ChangeBlock) => void,
): void {
  let i = 0;
    ...
    walkHunk(
      ...
      (block) => {
        block.dels.forEach((line, i) =>
          rows.push({ type: 'line', cell: { line, spans: block.delSpans[i] } }),
        );
        block.adds.forEach((line, i) =>
          rows.push({ type: 'line', cell: { line, spans: block.addSpans[i] } }),
        );
      },
    );
  });
