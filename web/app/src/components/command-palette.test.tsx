...
describe('Views group', () => {
  it('lists the 7 nav destinations plus New task with its C hint', async () => {
    ...
    await waitFor(() => expect(document.querySelectorAll('[data-slot="palette-view"]')).toHaveLength(8));
  ...
  it('drops the GitHub view when health reports no forge', async () => {
    ...
    await waitFor(() => expect(document.querySelectorAll('[data-slot="palette-view"]')).toHaveLength(7));
...
describe('Skills group', () => {
  ...
  it('orders local and team project skills before global ones, stably (#377/#555)', async () => {
    ...
    const names = [...document.querySelectorAll('[data-slot="palette-skill"]')].map((item) =>
      item.getAttribute('data-skill'),
    );
